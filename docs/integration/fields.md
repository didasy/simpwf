...
| `order`                          | Allowlisted field + optional single leading `-`. Default `-created_at`. `--x` rejected. Definitions: `id name version lineage_id [type] created_at updated_at` (`type` node-only; rejected on workflow list). Instances: `id workflow_definition_id status created_at updated_at` (ties break by `id` ascending). Statistics summary: `date` / `-date` only, default `-date` (bucket direction, not row sort). |
| `created_from` / `created_to`      | Optional RFC3339 timestamps (statistics summary only). Inclusive window on instance `created_at` (`>=` / `<=`). Garbage format or `from` after `to` → `400`.                                                                                                                                                                                                                                           |
...
| `input`         | `channel` (required: `http`, `redis`, `rabbitmq`). `output_property` (optional bare key, blank = node id). `form` (optional: `{schema (required, non-empty JSON Schema object, draft 2020-12, compile-checked), ui (optional JSON object of render hints)}`; input-only, rejected on references and other types). `validation.script` (optional, non-blank). `next_node`, hooks. `context_path` rejected (input-only parse error; use `output_property`). | External payload: any valid JSON (object, array, scalar) delivered on `channel` (`PUT .../input` for `http`; broker feed for `redis`/`rabbitmq`). Schema-first enforcement: when `form` is set, the payload is validated against `form.schema` before the script runs; schema failure rejects with `accepted: false` and a `schema validation failed: ...` error (HTTP 422; script never runs). Reusing an `Idempotency-Key` with a different payload returns 409; a corrected payload needs a fresh key. Validation script sees frozen `context` plus the raw payload bytes as string `input` var (so `JSON.parse(input)` first for objects). Non-empty string return = reject with that message (`accepted: false`). Status `pending_input` (`node_id`, `channel`, `output_property`, `form` with exact bytes; `form: null` when unset; omitted unless parked on input) feeds one generic frontend renderer per workflow version. | Accepted payload itself, written verbatim to `output_property` (context write is the effect). `attempt.output` = delivered payload JSON. No separate output object; downstream nodes read the value at `output_property`. Rejected payload: `attempt.output` = `null`, error = rejection message.                                                                                                                                                                            |
...
non-negative. Empty path, empty segments, and out-of-range array writes are rejected. Used by `input_data`, output
...
- Statistics summary: `success_rate` = finished / terminal (finished+failed+stopped), `null` when terminal is 0.
  ...
  `runs_per_day[].date` is `YYYY-MM-DD`; the array is `[]` when empty, never `null`.
...
- `input`: the accepted payload verbatim, written verbatim to `output_property` (blank = node id). Rejected delivery: `attempt.output` = `null`, `error` = rejection message.
