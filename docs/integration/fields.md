...
| `input`         | `channel` (required: `http`, `redis`, `rabbitmq`). `context_path` (required). `form` (optional: `{schema (required, non-empty JSON Schema object, draft 2020-12, compile-checked), ui (optional JSON object of render hints)}`; input-only, rejected on references and other types). `validation.script` (optional, non-blank). `next_node`, hooks. `output_property` accepted but ignored. | External payload: any valid JSON (object, array, scalar) delivered on `channel` (`PUT .../input` for `http`; broker feed for `redis`/`rabbitmq`). Schema-first enforcement: when `form` is set, the payload is validated against `form.schema` before the script runs; schema failure rejects with `accepted: false` and a `schema validation failed: ...` error (HTTP 422; script never runs). Reusing an `Idempotency-Key` with a different payload returns 409; a corrected payload needs a fresh key. Validation script sees frozen `context` plus the raw payload bytes as string `input` var (so `JSON.parse(input)` first for objects). Non-empty string return = reject with that message (`accepted: false`). Status `pending_input` (`node_id`, `channel`, `context_path`, `form` with exact bytes; `form: null` when unset; omitted unless parked on input) feeds one generic frontend renderer per workflow version. | Accepted payload itself, written verbatim to `context_path` (context write is the effect). `attempt.output` = delivered payload JSON. No separate output object; downstream nodes read the value at `context_path`. Rejected payload: `attempt.output` = `null`, error = rejection message.                                                                                                                                                                                  |
